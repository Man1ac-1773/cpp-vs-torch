...
Here is the Branch Execution data for an $N=2048$ matrix multiplication on the `performance-plugged` power profile:
...
> [!IMPORTANT]
> **Battery Profile Dependency**: Hardware-level measurements like branch prediction are highly sensitive to the OS power governor. Running this on a `saver` profile aggressively migrates the thread to slower E-Cores which have entirely different (and smaller) branch predictor units (BPU) than the P-Cores. All low-level PMU profiling must strictly lock the power profile to ensure the workload remains pinned to the correct CPU die architecture.

### The Hardware Mechanism: Loop Unrolling and the L1i Cache
