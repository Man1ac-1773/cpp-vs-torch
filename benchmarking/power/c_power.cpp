...
void run_mnist_power_bench(ofstream& json_out)
{
    ...
    for (uint i = 0; i < INPUT_DIM * HIDDEN_DIM; i++)
        model.fc1.weight->data[i] = 0.01f;
    for (uint i = 0; i < HIDDEN_DIM * OUTPUT_DIM; i++)
        model.fc2.weight->data[i] = -0.01f;
...
int main(int argc, char* argv[])
{
    ...
    string filepath = "./data/c_" + mode + ".jsonl";
    ...
    for (size_t N = 10; N <= 1000; N += step)
    {
        ...
        json_out << "{\"task\": \"matmul\", \"N\": " << N
                 << ", \"engine\": \"macrograd\", \"energy_joules\": " << c_energy << "}\n";
